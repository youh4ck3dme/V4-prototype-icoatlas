...
def enrich_address_with_region(
    ...
    if address:
        ...
        # Odstrániť PSČ (skúsme s medzerou aj bez)
        address_clean = re.sub(r"(\b\d{5}\b|\b\d{3}\s\d{2}\b)", "", address)
        # Zvláštny prípad: ak nie sú čiarky, skúsme vziať posledné "slovo" ako mesto
        if "," not in address_clean:
            words = address_clean.strip().split()
            if words:
                result["city"] = words[-1]
        else:
